...
{
	private static readonly ReadOnlyMemory<char> StartOfJson =
		"<script id=\"__NEXT_DATA__\" type=\"application/json\">".AsMemory();
	...
	private static string ToSynopsis(this JsonNode node)
	{
		return HttpUtility.HtmlDecode(
				node["props"]["pageProps"]["contentData"]["data"]["title"]["plotSynopsis"]["edges"][0]["node"]
					["plotText"]["plaidHtml"].ToString())
			.Replace("<br/>", " ")
			.Replace("\n", " ")
			.Replace("  ", " ");
	}
	...
	{
		return int.Parse(
			node["props"]["pageProps"]["contentData"]["entityMetadata"]["series"]["episodeNumber"]["episodeNumber"]
				.ToString());
	}
